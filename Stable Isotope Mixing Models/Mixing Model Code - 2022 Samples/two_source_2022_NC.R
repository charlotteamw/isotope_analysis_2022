...
df_isotopes <- read.csv("..//Isotope data/all_data_aly_char.csv", header = TRUE)
...
si_mixing_model_pred_bound <- function(x) {
  ...
  for(i in 1:nrow(df_pred)){
    ...
    TP_pred <- 2 + (((lake_coupling * ((df_pred$d_15N - lk_mean_dN)/3.4))) + ((1 - lake_coupling) * ((df_pred$cal.d15N - cr_mean_dN)/3.4)))
